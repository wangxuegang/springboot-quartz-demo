...
import com.wangxuegang.entity.PageBean;
...
public class JobServiceImpl implements JobService {
	...
	public Result listQuartzEntity(QuartzEntity quartz,
        ...
        Long totalCount = baseDao.nativeQueryCount(countSql);
        ...
        if(totalCount>0){
            ...
            List<QuartzEntity> list = baseDao.nativeQueryPagingList(QuartzEntity.class,pageable, nativeSql.toString(), params);
	...
	public Long listQuartzEntity(QuartzEntity quartz) {
		...
		nativeSql.append("WHERE tri.TRIGGER_TYPE = 'CRON'");
		return baseDao.nativeQueryCount(nativeSql.toString(), new Object[]{});
	}
    ...
    public Result save(QuartzEntity quartz) throws Exception{
        ...
        if(scheduler.checkExists(JobKey.jobKey(quartz.getJobName(), quartz.getJobGroup()))) return Result.error(quartz.getJobName()+":job已存在");
